...
/* These constructors create the object at the address pointed by dest instead 
 * of alloc-ing and returning addresses to new objects.
 *
 * Generally if these constructors are used(as in auto_array), nop_destructor() 
 * should be used. However, if certain postprocessing needs to be done, e.g 
 * destroying mutex locks, the user can still pass in a destructor that does 
 * not free memory pointed by elem
 */
